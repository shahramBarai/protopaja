...
int mSecondsToStart = 0;
int mSecondsOnline = 0;
int mSecondsOnlineRequest = 0;
...
void loop(){
  ...
  if (timerRunning == true){
    if(mSecondsToStart > 0){
      mSecondsToStart--;
    }else if(mSecondsOnline > 0){
      mSecondsOnline--;
      //This is the time when it should be online
    }else{
      timerRunning = false;
      mSecondsToStart = 0;
      mSecondsOnline = 0;
...
//Minutes to start
BLYNK:WRITE(V7){
  int getValue = param.asInt();
  //Timer starts running if it is not already running when minutes are given
  if (getValue > 0 && timerRunning == false){
    timerRunning = true;
    mSecondsToStart = getValue*60;
    mSecondsOnline = mSecondsOnlineRequest*60;
  }
}

//Minutes online
BLYNK:WRITE(V8){
  mSecondsOnlineRequest = param.asInt();
}
